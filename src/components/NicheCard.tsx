...
export default function NicheCard({ niche }: { niche: Niche }) {
  return (
    <Link
      href={`/${niche.slug}`}
      className="group rounded-2xl border border-zinc-200 bg-white p-5 hover:border-zinc-300 hover:shadow-sm transition"
    >
      <div className="text-xs uppercase tracking-widest text-zinc-500">{niche.shortTitle}</div>
