...
import { treatments, doctors } from "@/lib/mocks/clinic1";

export default function Page(){
  const sp=useSearchParams();
  const initTreatment = sp.get("treatment")||"";
  const initDoctor = sp.get("doctor")||"";
  const [step,setStep]=useState(initTreatment?2:1);
  ...
  async function confirm(){
    if(!phone||!name||!consent){alert("Name, phone, consent required"); return;}
    setPosting(true);
    try{
      await fetch("/api/book",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({demo:"clinic-1",serviceSlug:treatment,staffId:doctor,date,slot,name,phone})});
      setDone(true);
      const msg=`Hi SmileCare DHCC — appointment ${t?.name||treatment} on ${date} ${slot} with ${d?.name||"any doctor"} for ${name} (${phone})`;
  ...
  if(done) return (
    <div className="mx-auto max-w-xl px-6 py-12 text-center">
      <div className="bg-white border rounded-2xl p-8">
        ...
        <p className="text-sm text-zinc-600 mt-2">We posted to leads and opened WhatsApp. DHA licensed.</p>
        <Link href="/demos/clinic-1" className="mt-4 inline-flex px-6 py-3 bg-zinc-900 text-white rounded-full font-bold">Back to SmileCare</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-8">
      <h1 className="text-3xl font-serif font-bold">Request appointment — AED 150</h1>
      <div className="mt-2 text-xs text-zinc-500">Trust before the chair • DHA licensed • WhatsApp 30s</div>
      ...
      {step===1 && (
        <div className="mt-6 bg-white border rounded-2xl p-5">
          <div className="font-bold">Pick treatment</div>
          <div className="mt-3 grid gap-2">
            {treatments.map(x=>(
              <button key={x.slug} onClick={()=>{setTreatment(x.slug); setStep(2)}} className={`text-left border rounded-xl p-4 flex justify-between hover:border-zinc-900 ${treatment===x.slug?"border-zinc-900 bg-blue-50":"bg-white"}`}>
      ...
      {step===2 && (
        <div className="mt-6 bg-white border rounded-2xl p-5">
          ...
          <div className="mt-3 grid md:grid-cols-2 gap-3">
            <button onClick={()=>{setDoctor(""); setStep(3)}} className={`border rounded-2xl p-4 text-left ${!doctor?"border-zinc-900 bg-blue-50":"bg-white"}`}><div className="font-bold">Any doctor</div><div className="text-xs text-zinc-500">We pick by treatment</div></button>
            {doctors.filter(doc=> !treatment || doc.treatments.includes(treatment)).map(doc=>(
              <button key={doc.id} onClick={()=>{setDoctor(doc.id); setStep(3)}} className={`border rounded-2xl p-4 flex gap-3 text-left hover:border-zinc-900 ${doctor===doc.id?"border-zinc-900 bg-blue-50":"bg-white"}`}>
                <img src={doc.image} alt={doc.name} className="h-12 w-12 rounded-full object-cover border" />
                <div><div className="font-bold">{doc.name}</div><div className="text-xs text-zinc-500">{doc.role} • {doc.creds}</div></div>
              </button>
            ))}
          </div>
      ...
      {step===4 && (
        <div className="mt-6 bg-white border rounded-2xl p-5">
          ...
          <div className="mt-4 space-y-3">
            ...
            <label className="flex gap-2 text-xs"><input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)} /> I consent to photos being used only with written permission.</label>
          </div>
          <div className="mt-4 bg-zinc-50 border rounded-xl p-4 text-sm">
            <div><span className="font-bold">Treatment:</span> {t?.name||"—"}</div>
