...
import { listings } from "@/lib/mocks/realestate1";
export default function Page(){
  ...
  async function confirm(){
    ...
    setPosting(true);
    try{
      await fetch("/api/book",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({demo:"realestate-1",serviceSlug:listing,date,slot,name,phone})});
      setDone(true);
      const msg=`Hi Vista — viewing for ${l?.title||listing} on ${date} ${slot} for ${name} (${phone})`;
      window.open(`https://wa.me/919999999999?text=${encodeURIComponent(msg)}`,"_blank");
    }finally{setPosting(false);}
  }
  if(done) return (<div className="mx-auto max-w-xl px-6 py-12 text-center"><div className="bg-white border rounded-2xl p-8"><div className="text-2xl">✓</div><h1 className="text-2xl font-bold mt-2">Viewing requested!</h1><Link href="/demos/realestate-1" className="mt-4 inline-flex px-6 py-3 bg-zinc-900 text-white rounded-full font-bold">Back to Vista</Link></div></div>);
  return (
    <div className="mx-auto max-w-3xl px-6 py-8">
      <h1 className="text-3xl font-serif font-bold">Book viewing — Vista</h1>
