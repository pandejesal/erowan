...
import { areas, getArea, listings } from "@/lib/mocks/realestate1";
export function generateStaticParams(){ return areas.map(a=>({slug:a.slug})); }
export function generateMetadata({params}:{params:{slug:string}}){
  const a=getArea(params.slug);
  if(!a) return {};
  return { title: `${a.name} — ${a.avgPrice} | Vista Homes`, description: a.desc, alternates:{canonical:`https://erowan.com/demos/realestate-1/areas/${a.slug}`} };
}
export default function Page({params}:{params:{slug:string}}){
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-8">
      <div className="text-xs text-zinc-500"><Link href="/demos/realestate-1/areas" className="underline">Areas</Link> / {a.name}</div>
