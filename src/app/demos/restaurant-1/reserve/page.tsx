...
export default function Page(){
  ...
  async function confirm(){
    ...
    setPosting(true);
    try{
      await fetch("/api/book",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({demo:"restaurant-1",date,slot:time,name,phone,items:`Table for ${guests}`})});
      setDone(true);
      const msg=`Hi Mira JLT — reserve table for ${guests} on ${date} ${time} for ${name} (${phone})`;
  ...
  if(done) return (
    <div className="mx-auto max-w-xl px-6 py-12 text-center">
      <div className="bg-white border rounded-2xl p-8">
        ...
        <Link href="/demos/restaurant-1" className="mt-4 inline-flex px-6 py-3 bg-zinc-900 text-white rounded-full font-bold">Back to Mira</Link>
      </div>
    </div>
  );
