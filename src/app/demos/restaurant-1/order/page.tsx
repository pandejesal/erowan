...
export default function Page(){
  ...
  useEffect(()=>{
    const s=localStorage.getItem("mira-cart");
    ...
    if(q){
      ...
      setCart(next);
      localStorage.setItem("mira-cart",JSON.stringify(next));
      window.history.replaceState({},'',"/demos/restaurant-1/order");
    }
  },[]);

  useEffect(()=>{ localStorage.setItem("mira-cart",JSON.stringify(cart)); },[cart]);
  ...
  async function checkout(){
    ...
    await fetch("/api/book",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({demo:"restaurant-1",name,phone,items:cart})});
    const msg=`Hi Mira JLT — order: ${items.map(i=>`${i.qty}x ${i.m!.name}`).join(", ")} — AED ${subtotal} for ${name} (${phone})`;
    window.open(`https://wa.me/919999999999?text=${encodeURIComponent(msg)}`,"_blank");
    setCart([]);
    localStorage.removeItem("mira-cart");
  }

  return (
    <div className="mx-auto max-w-3xl px-6 py-8">
      <h1 className="text-3xl font-serif font-bold">Order — Mira JLT</h1>
      ...
      {items.length===0 ? (
        <div className="mt-6 bg-white border rounded-2xl p-8 text-center">
          ...
          <Link href="/demos/restaurant-1/menu" className="mt-3 inline-flex px-6 py-3 bg-zinc-900 text-white rounded-full font-bold">See menu →</Link>
        </div>
      ) : (
        <div className="mt-6 bg-white border rounded-2xl p-5">
          ...
          <div className="mt-4 border-t pt-4 flex justify-between font-bold">Subtotal<div>AED {subtotal}</div></div>
          <div className="mt-4 space-y-3">
            ...
            <button onClick={checkout} className="w-full px-6 py-3 bg-zinc-900 text-white rounded-full font-bold">Checkout on WhatsApp → AED {subtotal}</button>
          </div>
        </div>
      )}
