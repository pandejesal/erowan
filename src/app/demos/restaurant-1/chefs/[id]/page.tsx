...
import { chefs, getChef, menu } from "@/lib/mocks/restaurant1";

export function generateStaticParams(){ return chefs.map(c=>({id:c.id})); }
export function generateMetadata({params}:{params:{id:string}}){
  const c=getChef(params.id);
  if(!c) return {};
  return { title:`${c.name} — ${c.role} | Mira Cafe`, description:c.bio, alternates:{canonical:`https://erowan.com/demos/restaurant-1/chefs/${c.id}`} };
}
