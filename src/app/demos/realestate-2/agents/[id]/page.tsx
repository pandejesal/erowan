...
import { agents, getAgent, listings } from "@/lib/mocks/realestate2";
export function generateStaticParams(){ return agents.map(a=>({id:a.id})); }
export function generateMetadata({params}:{params:{id:string}}){
  const a=getAgent(params.id);
  if(!a) return {};
  return { title: `${a.name} — ${a.role} | Harbor`, description: a.bio, alternates:{canonical:`https://erowan.com/demos/realestate-2/agents/${a.id}`} };
}
export default function Page({params}:{params:{id:string}}){
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-8">
      <div className="text-xs text-zinc-500"><Link href="/demos/realestate-2/agents" className="underline">Agents</Link> / {a.name}</div>
      <div className="mt-4 grid md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <div className="mt-3 grid gap-3">{myListings.map(l=>(
            <Link key={l.id} href={`/demos/realestate-2/listings/${l.id}`} className="bg-white border rounded-2xl overflow-hidden flex hover:shadow-sm">
