...
import { posts, getPost } from "@/lib/mocks/realestate2";
export function generateStaticParams(){ return posts.map(p=>({slug:p.slug})); }
export function generateMetadata({params}:{params:{slug:string}}){
  const p=getPost(params.slug);
  if(!p) return {};
  return { title: `${p.title} | Harbor Blog`, description: p.excerpt, alternates:{canonical:`https://erowan.com/demos/realestate-2/blog/${p.slug}`} };
}
export default function Page({params}:{params:{slug:string}}){
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-8">
      <div className="text-xs text-zinc-500"><Link href="/demos/realestate-2/blog" className="underline">Blog</Link> / {p.title}</div>
