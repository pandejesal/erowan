...
import { services, therapists } from "@/lib/mocks/salon2";
...
export default function Page(){
  ...
  const staff=therapists.find(s=>s.id===staffId);

  async function confirm(){
    if(!phone||!name){alert("Name + phone required"); return;}
    setPosting(true);
    try{
      await fetch("/api/book",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({demo:"salon-2",serviceSlug,staffId,date,slot,name,phone})});
      setDone(true);
      const msg=`Hi Bloom — booking ${service?.name||serviceSlug} on ${date} ${slot} with ${staff?.name||"any therapist"} for ${name} (${phone}) — EN+ES`;
  ...
  if(done) return (
    <div className="mx-auto max-w-xl px-6 py-12 text-center">
      <div className="bg-white border rounded-2xl p-8">
        ...
        <Link href="/demos/salon-2" className="mt-4 inline-flex px-6 py-3 bg-zinc-900 text-white rounded-full font-bold">Back to Bloom</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-8">
      <h1 className="text-3xl font-serif font-bold">Book — Bloom</h1>
      ...
      {step===2 && (
        <div className="mt-6 bg-white border rounded-2xl p-5">
          <div className="font-bold">Pick therapist</div>
          <div className="mt-3 grid md:grid-cols-2 gap-3">
            <button onClick={()=>{setStaffId(""); setStep(3)}} className={`border rounded-2xl p-4 text-left ${!staffId?"border-zinc-900 bg-zinc-50":"bg-white"}`}><div className="font-bold">Any therapist</div><div className="text-xs text-zinc-500">We pick</div></button>
            {therapists.map(t=>(
              <button key={t.id} onClick={()=>{setStaffId(t.id); setStep(3)}} className={`border rounded-2xl p-4 flex gap-3 text-left hover:border-zinc-900 ${staffId===t.id?"border-zinc-900 bg-zinc-50":"bg-white"}`}>
                <img src={t.image} alt={t.name} className="h-12 w-12 rounded-full object-cover border" />
                <div><div className="font-bold">{t.name}</div><div className="text-xs text-zinc-500">{t.specialty} • {t.languages.join("+")}</div></div>
              </button>
            ))}
          </div>
      ...
      {step===4 && (
        <div className="mt-6 bg-white border rounded-2xl p-5">
          ...
          <div className="mt-4 bg-[#E8F0E8] border rounded-xl p-4 text-sm">
            <div><span className="font-bold">Service:</span> {service?.name||"—"}</div>
            <div><span className="font-bold">Therapist:</span> {staff?.name||"Any"}</div>
